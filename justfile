...
install m:
    #!/usr/bin/env bash
    set -euo pipefail
    if [[ ! -x "build-{{m}}/motion" ]]; then
        echo "error: build-{{m}}/motion is missing; run 'just build {{m}}' before installing" >&2
        exit 1
    fi
...
_clang_tidy m=mode *args:
    ...
    # sort main-app (motion.p) entries first, then keep the first entry per file
    python3 -c "import json, sys; e = sorted(json.load(open(sys.argv[1])), key=lambda x: not x.get('output', '').startswith('motion.p/')); b = {}; [b.setdefault(x['file'], x) for x in e]; json.dump(list(b.values()), open(sys.argv[2], 'w'))" "build-{{m}}/compile_commands.json" "$cdb_dir/compile_commands.json"
