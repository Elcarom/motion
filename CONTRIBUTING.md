...
This file collects contributor-facing details for Motion: design goals, stack notes, code style, source layout,
...
Motion needs the `assets/` tree at runtime. Copying only the bare `motion` binary is not enough.
...
4. install-style `../share/motion/assets` relative to the executable
5. the compiled install path from Meson (`<prefix>/<datadir>/motion/assets`)
...
`assets/translations/en.json` is the source catalog for new user-facing strings. The other JSON files are inherited
locale catalogs and should be updated through an explicit translation review rather than machine translation.

For normal code and UI changes, update the English catalog and run:

```sh
python3 tools/i18n-check.py
```

Do not copy English text into every locale. Non-English edits should be made by translators or as part of a documented
catalog import/export. Product renames must preserve locale JSON validity and should avoid inventing URLs or services
that the fork does not operate.
...
All debug commands use the `dev.motion.Debug` D-Bus service, available at runtime.

```sh
# Enable verbose debug logs
gdbus call --session --dest dev.motion.Debug --object-path /dev/motion/Debug --method dev.motion.Debug.SetVerboseLogs true

# Disable verbose debug logs
gdbus call --session --dest dev.motion.Debug --object-path /dev/motion/Debug --method dev.motion.Debug.SetVerboseLogs false

# Check current verbose log state
gdbus call --session --dest dev.motion.Debug --object-path /dev/motion/Debug --method dev.motion.Debug.GetVerboseLogs

# Emit an internal notification (app_name, summary, body, timeout_ms, urgency 0-2)
gdbus call --session --dest dev.motion.Debug --object-path /dev/motion/Debug --method dev.motion.Debug.EmitInternalNotification "Motion" "Test" "Hello from debug" 5000 1
```
