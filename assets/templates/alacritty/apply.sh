...
if [[ "$config_dir" == "$HOME"/* ]]; then
    theme_path="~/${config_dir#"$HOME"/}/alacritty/themes/motion.toml"
else
    theme_path="$config_dir/alacritty/themes/motion.toml"
fi
...
if grep -q 'motion\.toml' "$config_file"; then
    sed -i 's|"themes/motion.toml"|"'"$theme_path"'"|g' "$config_file"
