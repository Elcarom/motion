...
scheme_line='config.color_scheme = "Motion"'
...
if ! grep -q "^\s*config\.color_scheme\s*=\s*['\"]Motion['\"]\s*" "$config_file"; then
    if grep -q '^\s*config\.color_scheme\s*=' "$config_file"; then
        sed -i "s|^\(\s*config\.color_scheme\s*=\s*\).*$|\1\"Motion\"|" "$config_file"
