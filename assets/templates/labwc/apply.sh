...
theme_dir="${XDG_DATA_HOME:-$HOME/.local/share}/themes/motion/openbox-3"
theme_file="$theme_dir/themerc"
source_theme="$config_dir/motion.conf"
...
if grep -q '<theme>' "$rc_file"; then
    ...
    if sed -n '/<theme>/,/<\/theme>/p' "$rc_file" | grep -q '<name>.*</name>'; then
        sed -i '/<theme>/,/<\/theme>/s|<name>.*</name>|<name>motion</name>|' "$rc_file"
    else
        sed -i '/<theme>/a\    <name>motion</name>' "$rc_file"
    fi
else
