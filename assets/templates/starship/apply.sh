...
palette_file="${XDG_CACHE_HOME:-$HOME/.cache}/motion/starship-palette.toml"
marker_begin="# >>> MOTION STARSHIP PALETTE >>>"
marker_end="# <<< MOTION STARSHIP PALETTE <<<"
...
discover_starship_config() {
    ...
    # Motion applies templates from its daemon, which does not inherit shell-only
...
if [ ! -f "$config_file" ]; then
    echo 'palette = "motion"' >"$config_file"
else
    if grep -qE '^palette\s*=' "$config_file"; then
        sed -i -E 's/^palette\s*=.*/palette = "motion"/' "$config_file"
    elif grep -qE '^"\$schema"' "$config_file"; then
        sed -i '/^"\$schema"/a palette = "motion"' "$config_file"
    else
        sed -i '1i palette = "motion"' "$config_file"
    fi
