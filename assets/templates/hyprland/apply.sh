...
lua_output_file="$config_dir/motion.lua"
conf_output_file="$config_dir/motion.conf"
...
if [[ "$config_dir" == "$HOME"/* ]]; then
  conf_source_path="~/${config_dir#"$HOME"/}/motion.conf"
else
...
detect_mode() {
  ...
  # Fallback for cases where motion applies templates where hyprland
...
apply_lua() {
  local include_line='-- For Motion Color templates
require("motion").apply_theme()'
  ...
  # Append only if there is no Motion include at all
  if ! grep -qF 'require("motion")' "$lua_config_file"; then
    printf '\n%s\n' "$include_line" >>"$lua_config_file"
  fi
}
...
source = $conf_source_path"
  ...
  # Avoid appending duplicate Motion includes
  if ! grep -qE 'source\s*=.*motion\.conf' "$conf_config_file"; then
    printf '\n%s\n' "$include_line" >>"$conf_config_file"
  fi
}
