...
void Application::initIpc() {
  ...
  m_ipcService.registerHandler(
      ...
      "notification-show <summary [-- body]|json>", "Show an internal Motion notification"
  );
  ...
  m_ipcService.registerHandler(
      ...
      [this](const std::string& args) -> std::string {
        if (!motion::ipc::splitWords(args).empty()) {
  ...
  m_ipcService.registerHandler(
      ...
      [this](const std::string& args) -> std::string {
        ...
        const auto value = motion::ipc::parseNormalizedOrPercent(parts[0]);
        if (!value.has_value()) {
