...
void Application::syncNotificationDaemon() {
  ...
  try {
    ...
    if (compositors::isKde()) {
      try {
        ...
        m_notificationManager.addInternal(
            "Motion", i18n::tr("notifications.internal.dbus-disabled"), kdeError.what(), Urgency::Low
        );
        return;
      }
    }
    ...
    m_notificationDaemonInitFailed = true;
    m_notificationManager.addInternal(
        "Motion", i18n::tr("notifications.internal.dbus-disabled"), ownerError.what(), Urgency::Low
    );
  }
}
    ...
    if (!ok) {
      ...
      DeferredCall::callLater([this, error]() {
        if (polkit_session::isNoSessionForPidError(error)) {
          notify::error(
              "Motion", i18n::tr("notifications.internal.polkit-agent"),
...
void Application::initStyleThemeAndWayland() {
  ...
  // Publish the connected outputs to plugin scripts (motion.outputs()), refreshed on every
  ...
  m_themeService.setResolvedCallback([this, lastResolvedThemeMode = std::optional<std::string>{},
                                      syncScriptApiWallpaperDirectory](
                                         const motion::theme::GeneratedPalette& generated, std::string_view mode
                                     ) mutable {
    ...
    m_templateApplyService.setAfterApplyCallback([this, resolvedMode, previousMode, configuredMode]() {
      ...
      if (previousMode.has_value() && *previousMode != resolvedMode) {
        m_hookManager.fire(
            ...
            {{"MOTION_THEME_MODE", resolvedMode},
             {"MOTION_THEME_MODE_PREVIOUS", *previousMode},
             {"MOTION_THEME_MODE_CONFIGURED", configuredMode}}
        );
      }
    });
...
void Application::initAuxServicesAndHooks() {
  ...
  m_configService.setWallpaperChangeCallback([this]() {
    ...
    const auto fireWallpaperChangedHook = [this](const std::string& path, const std::string& connector) {
      m_hookManager.fire(
          HookKind::WallpaperChanged, {{"MOTION_WALLPAPER_PATH", path}, {"MOTION_WALLPAPER_CONNECTOR", connector}}
      );
    };
...
void Application::initSessionBusServices() {
  ...
  try {
    ...
    m_notificationManager.addInternal(
        "Motion", i18n::tr("notifications.internal.session-bus-unavailable"), e.what(), Urgency::Low
    );
  }
  ...
  if (m_bus != nullptr) {
    try {
      ...
      kLog.info("debug service active on dev.motion.Debug");
    } catch (const std::exception& e) {
    ...
    try {
      ...
      m_notificationManager.addInternal(
          "Motion", i18n::tr("notifications.internal.mpris-disabled"), e.what(), Urgency::Low
      );
    }
