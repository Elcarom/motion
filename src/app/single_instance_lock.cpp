...
bool SingleInstanceLock::tryAcquire() {
  ...
  const int fd = ::open(m_path.c_str(), O_RDWR | O_CREAT | O_CLOEXEC | O_NOFOLLOW, 0600);
  if (fd < 0) {
...
std::string SingleInstanceLock::resolveLockPath() { return motion::runtime_path::path("lock"); }
