...
namespace {
  ...
  const sdbus::ServiceName kBusName{"dev.motion.KWinActiveWindow"};
  const sdbus::ObjectPath kObjectPath{"/dev/motion/KWinActiveWindow"};
  constexpr auto kInterface = "dev.motion.KWinActiveWindow";
  constexpr auto kScriptLabel = "motion-active-window";
...
const BUS = "dev.motion.KWinActiveWindow";
const PATH = "/dev/motion/KWinActiveWindow";
const IFACE = "dev.motion.KWinActiveWindow";
...
function notify(window) {
  try {
    ...
    print("motion-active-window notify failed: " + error);
  }
}

function isMotionShellSurface(window) {
  const resourceClass = (window.resourceClass || "").toLowerCase();
  if (resourceClass === "motion") {
    return true;
  }
  const caption = (window.caption || "").toLowerCase();
  return caption === "motion" && resourceClass === "";
}
...
function shouldTrack(window) {
  ...
  if (isMotionShellSurface(window)) {
    return false;
  }
  return true;
}
...
function syncWindows() {
  try {
    ...
    print("motion-active-window sync failed: " + error);
  }
}
...
syncWindows();
  ...
  [[nodiscard]] bool isMotionShellSurface(const std::string& appId, const std::string& title) {
    const std::string appLower = StringUtils::toLower(appId);
    if (appLower == "motion") {
      return true;
    }
    if (appLower.empty() && StringUtils::toLower(title) == "motion") {
      return true;
    }
    return false;
  }
...
namespace compositors::kde {
  ...
  std::vector<std::string> KwinActiveWindow::runningAppIds() const {
    ...
    for (const auto& window : m_trackedWindows) {
      if (window.appId.empty()
          || isMotionShellSurface(window.appId, window.title)
  ...
  KwinActiveWindow::windowsForApp(const std::string& idLower, const std::string& wmClassLower) const {
    ...
    for (const auto& window : m_trackedWindows) {
      if (window.appId.empty() || isMotionShellSurface(window.appId, window.title)) {
        continue;
      }
  ...
  std::vector<WorkspaceWindow> KwinActiveWindow::trackedWorkspaceWindows() const {
    ...
    for (const auto& window : m_trackedWindows) {
      ...
      if (appId.empty() || isMotionShellSurface(appId, title)) {
        continue;
      }
...
for (const window of workspace.windowList()) {{
  ...
  if (window.resourceClass === targetClass && window.caption === targetCaption) {{
    ...
    const std::string label = std::format("motion-activate-{}", ++m_transientScriptSerial);
...
for (const window of workspace.windowList()) {{
  ...
  break;
    ...
    const std::string label = std::format("motion-close-{}", ++m_transientScriptSerial);
    if (!runTransientScript(script, label)) {
  ...
  void KwinActiveWindow::onNotifyActiveWindow(
    ...
    if (caption.empty() && resourceClass.empty()) {
      ...
      if (isMotionShellSurface(appId, title)) {
        return;
      }
  ...
  void KwinActiveWindow::onNotifyWindowList(const std::string& payload) {
    ...
    for (const auto& record : splitString(payload, kRecordSeparator)) {
      ...
      if (isMotionShellSurface(window.appId, window.title)) {
        continue;
      }
