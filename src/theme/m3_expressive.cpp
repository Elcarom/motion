...
namespace motion::theme {
  ...
  namespace {
    ...
    std::unique_ptr<mcu::DynamicScheme> makeScheme(const mcu::Hct& source, bool isDark) {
  ...
  GeneratedPalette generateExpressive(const std::vector<uint8_t>& rgb112) {
    ...
    auto darkScheme = makeScheme(source, true);
    auto lightScheme = makeScheme(source, false);
