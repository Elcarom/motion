...
namespace {
  ...
  constexpr const char* kScreenshotPathEnv = "MOTION_SCREENSHOT_PATH";
...
std::filesystem::path ScreenshotService::defaultPicturesDirectory() const {
  ...
  return std::filesystem::path(FileUtils::cacheDir()) / "screenshots";
}
...
void ScreenshotService::notifySaved(const std::filesystem::path& path) {
  m_notifications.addInternal("Motion", "Screenshot saved", path.string());
}

void ScreenshotService::notifyError(const std::string& message) {
  m_notifications.addInternal("Motion", "Screenshot failed", message, Urgency::Critical);
}
