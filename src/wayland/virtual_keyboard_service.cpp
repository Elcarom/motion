...
bool VirtualKeyboardService::ensureKeymap() {
  ...
  std::string keymapTemplate =
      (motion::runtime_path::trustedRuntimeDirectory()
       / ("motion-" + std::to_string(static_cast<unsigned long long>(::geteuid())) + "-virtual-keyboard-XXXXXX"))
          .string();
  std::vector<char> keymapPath(keymapTemplate.begin(), keymapTemplate.end());
  keymapPath.push_back('\0');
  const int fd = mkstemp(keymapPath.data());
  if (fd < 0) {
    std::free(keymapString);
    kLog.warn("failed to allocate temporary keymap file");
    return false;
  }
  unlink(keymapPath.data());
