...
namespace schema = motion::config::schema;
...
namespace {
  ...
  void readConfigSection(
      const toml::table& table, T& target, const motion::config::schema::Schema<T>& sectionSchema,
      std::string_view path, motion::config::schema::Diagnostics& diagnostics
  ) {
    T candidate = target;
    try {
      motion::config::schema::readInto(table, candidate, sectionSchema, path, diagnostics);
  ...
  void restoreInvalidComponents(
      Config& candidate, const Config& active, const motion::config::schema::Diagnostics& diagnostics
  ) {
    ...
    for (const auto& entry : diagnostics.entries) {
      if (entry.severity != motion::config::schema::Diagnostics::Severity::Error
          || entry.recoveryScope != motion::config::schema::Diagnostics::RecoveryScope::Component) {
        continue;
      }
  ...
  readDesktopWidgetState(std::string_view id, const toml::table& widgetTable, std::string_view colorSection) {
    ...
    if (const auto* settingsTable = widgetTable["settings"].as_table()) {
      for (const auto& [key, value] : *settingsTable) {
        if (auto parsed = motion::config::readWidgetSettingValue(value); parsed.has_value()) {
  ...
  mergeUserConfigSources(std::string_view configDir, std::string_view settingsPath, std::string* error) {
    auto mergeResult = motion::config::mergeConfigWithIncludes(configDir);
    ...
    if (!settingsPath.empty() && std::filesystem::exists(std::filesystem::path(std::string(settingsPath)))) {
      try {
        ...
        const auto storedVersion = motion::config::storedConfigVersion(sidecar, migrationDiag);
        if (storedVersion.has_value()) {
          (void)motion::config::applyPendingConfigMigrations(sidecar, *storedVersion, migrationDiag);
        }
  ...
  if (m_notificationManager != nullptr && !m_pendingError.empty()) {
    ...
    DeferredCall::callLater([this, pendingError]() {
      ...
      m_configErrorNotificationId =
          m_notificationManager->addInternal("Motion", "Config error", pendingError, Urgency::Critical, 0);
    });
  }
...
std::string ConfigService::buildSupportReport() const {
  ...
  report.insert_or_assign("generated_by", "motion");
  report.insert_or_assign("generated_at_utc", utcTimestamp());
  report.insert_or_assign("motion_version", std::string(motion::build_info::version()));
  report.insert_or_assign("git_revision", std::string(motion::build_info::revision()));
...
std::string ConfigService::buildMergedUserConfigFromSources(
  ...
  normalized.erase(motion::config::kConfigVersionKey);
  motion::config::LegacyConfigIssues issues;
  motion::config::normalizeLegacyConfig(normalized, issues);
...
std::string ConfigService::buildEffectiveConfigFromSources(
  ...
  Config config;
  motion::config::seedBuiltinWidgets(config);
  if (normalized.empty()) {
...
void ConfigService::setConfigParseError(std::string parseError) {
  ...
  if (m_notificationManager != nullptr) {
    ...
    m_configErrorNotificationId =
        m_notificationManager->addInternal("Motion", "Config error", parseError, Urgency::Critical, 0);
  } else {
    m_pendingError = std::move(parseError);
  }
}

void ConfigService::updateLegacyConfigIssues(motion::config::LegacyConfigIssues issues) {
  ...
  const std::string fingerprint = motion::config::legacyConfigIssueFingerprint(issues);
  ...
  const bool hasNewIssues = !reminderState.has_value()
      || motion::config::legacyConfigFingerprintHasNewIssues(fingerprint, reminderState->issueFingerprint);
  const bool intervalElapsed = !reminderState.has_value()
      || motion::config::legacyConfigReminderIntervalElapsed(now, reminderState->epochSeconds);
  ...
  const auto remaining = std::chrono::seconds(motion::config::kLegacyConfigReminderIntervalSeconds) - elapsed;
...
void ConfigService::notifyLegacyConfigIssues() {
  ...
  const std::string fingerprint = motion::config::legacyConfigIssueFingerprint(m_legacyConfigIssues);
  const std::int64_t now = currentEpochSeconds();
  (void)m_notificationManager->addInternal(
      "Motion", i18n::tr("notifications.internal.config-migration-title"),
  ...
  m_legacyReminderTimer.start(
      std::chrono::milliseconds(motion::config::kLegacyConfigReminderIntervalSeconds * 1000), [this]() {
        m_legacyReminderPending = true;
        notifyLegacyConfigIssues();
      }
  );
...
void ConfigService::loadAll() {
  motion::profiling::ScopedTimer parseTimer(kLog, "reload: parse (loadAll)");
  m_effectiveOverrideCache.clear();

  Config nextConfig;
  motion::config::seedBuiltinWidgets(nextConfig);

  auto mergeResult = motion::config::mergeConfigWithIncludes(m_configDir);
  toml::table merged = std::move(mergeResult.merged);
  ...
  int storedVersion = motion::config::currentConfigVersion();
  int appliedVersion = storedVersion;
  bool sidecarNeedsPersist = false;
  if (!m_overridesTable.empty()) {
    const auto parsedVersion = motion::config::storedConfigVersion(effectiveOverrides, migrationDiag);
  ...
  merged.erase(motion::config::kConfigVersionKey);
  motion::config::LegacyConfigIssues legacyIssues;
  motion::config::normalizeLegacyConfig(merged, legacyIssues);
  ...
  if (semanticError.empty()) {
    try {
      diagnostics = motion::config::validateMergedConfig(merged);
...
void ConfigService::parseConfigTable(
  ...
  if (auto* widgetTbl = tbl["widget"].as_table()) {
    for (const auto& [name, node] : *widgetTbl) {
      ...
      WidgetConfig wc = motion::config::readBarWidgetConfig(widgetName, *entryTbl, config);
