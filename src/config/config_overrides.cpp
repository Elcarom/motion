...
std::optional<Config> ConfigService::configForOverrides(const toml::table& overrides) const {
  Config parsed;
  motion::config::seedBuiltinWidgets(parsed);
  ...
  motion::config::schema::Diagnostics migrationDiag;
  if (!effectiveOverrides.empty()) {
    const auto storedVersion = motion::config::storedConfigVersion(effectiveOverrides, migrationDiag);
    if (storedVersion.has_value()) {
      (void)motion::config::applyPendingConfigMigrations(effectiveOverrides, *storedVersion, migrationDiag);
    }
  }
  ...
  motion::config::normalizeLegacyConfig(merged, issues);
...
motion::config::schema::Diagnostics ConfigService::diagnosticsForOverrides(const toml::table& overrides) const {
  auto mergeResult = motion::config::mergeConfigWithIncludes(m_configDir);
  toml::table merged = std::move(mergeResult.merged);
  motion::config::schema::Diagnostics diagnostics;
  if (!mergeResult.firstError.empty()) {
  ...
  toml::table effectiveOverrides = overrides;
  if (!effectiveOverrides.empty()) {
    const auto storedVersion = motion::config::storedConfigVersion(effectiveOverrides, diagnostics);
    if (storedVersion.has_value()) {
      (void)motion::config::applyPendingConfigMigrations(effectiveOverrides, *storedVersion, diagnostics);
    }
  }
  deepMerge(merged, effectiveOverrides);
  merged.erase(motion::config::kConfigVersionKey);
  ...
  auto semantic = motion::config::validateMergedConfig(merged);
...
bool ConfigService::validateOverrideMutation(
    ...
    const motion::config::schema::Diagnostics* candidateDiagnostics
) {
  ...
  try {
    ...
    const auto computedCandidate = candidateDiagnostics == nullptr ? diagnosticsForOverrides(candidateOverrides)
                                                                   : motion::config::schema::Diagnostics{};
    ...
    const auto fatal = std::ranges::find_if(candidate.entries, [](const auto& entry) {
      return entry.severity == motion::config::schema::Diagnostics::Severity::Error
          && entry.recoveryScope == motion::config::schema::Diagnostics::RecoveryScope::Document;
    });
...
bool ConfigService::validateOverride(
  ...
  const auto fieldError = std::ranges::find_if(candidateDiagnostics.entries, [&](const auto& entry) {
    return entry.severity == motion::config::schema::Diagnostics::Severity::Error && entry.path == settingPath;
  });
...
std::string ConfigService::firstRunWallpaperPath() const {
  ...
  const auto path = paths::assetPath("motion-wallpaper.png");
  std::error_code ec;
