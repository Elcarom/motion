...
namespace motion::config {
  namespace {
    ...
    constexpr const char* kHelpText =
        "Usage: motion config <command> [options]\n"
        "\n"
        "Commands:\n"
        "  validate [path]\n"
        ...
        "      Reconstruct config-home/motion and state-home/motion from a support report.\n"
        "\n"
        "  replay-report <report.toml> --target <dir> --flattened [--force]\n"
        "      Reconstruct a single config-home/motion/config.toml from the report's merged config.\n";

    constexpr const char* kValidateHelpText =
        "Usage: motion config validate [path]\n"
        "\n"
    ...
    constexpr const char* kReplayHelpText =
        "Usage: motion config replay-report <report.toml> --target <dir> [--flattened] [--force]\n"
        "\n"
        "Options:\n"
    ...
    constexpr const char* kExportHelpText = "Usage: motion config export [merged|full]\n"
                                            "\n"
    ...
    constexpr const char* kSettingsCountHelpText =
        "Usage: motion config settings-count\n"
        "\n"
    ...
    int runSettingsCount(int argc, char* argv[]) {
      for (int i = 3; i < argc; ++i) {
        ...
        std::println(stderr, "Run 'motion config settings-count --help' for usage.");
        return 1;
      }
    ...
    int replayReport(const ReplayOptions& options, const char* argv0) {
      ...
      const std::filesystem::path configDir = configHome / "motion";
      const std::filesystem::path stateDir = stateHome / "motion";
      ...
      std::println(
          "  MOTION_CONFIG_HOME={} MOTION_STATE_HOME={} {}", StringUtils::shellQuote(configHome.string()),
    ...
    int runValidate(int argc, char* argv[]) {
      ...
      for (int i = 3; i < argc; ++i) {
        ...
        std::println(stderr, "error: unexpected argument: {}", argv[i]);
        std::println(stderr, "Run 'motion config validate --help' for usage.");
        return 1;
      }
    ...
    int runExport(int argc, char* argv[]) {
      ...
      for (int i = 3; i < argc; ++i) {
        ...
        std::println(stderr, "error: unexpected argument: {}", argv[i]);
        std::println(stderr, "Run 'motion config export --help' for usage.");
        return 1;
      }
  ...
  int runCli(int argc, char* argv[]) {
    ...
    if (std::strcmp(argv[2], "replay-report") == 0) {
      ...
      if (!parsed) {
        ...
        std::println(stderr, "Run 'motion config replay-report --help' for usage.");
        return 1;
      }
    ...
    std::println(stderr, "Run 'motion config --help' for usage.");
    return 1;
  }
