...
struct ShellSessionConfig {
  ...
  struct ShellSessionPowerConfig {
    ...
    // When unset, Motion tries a prioritized backend list (systemd/logind/privileged helpers).
...
struct ShellConfig {
  ...
  bool telemetryEnabled = false; // legacy compatibility only; Motion never transmits telemetry
  ...
  int clipboardHistoryMaxEntries = static_cast<int>(motion::config::kClipboardHistoryDefaultEntries);
...
struct SystemConfig {
  struct MonitorConfig {
    ...
    double netTxCriticalThreshold = motion::sysmon::thresholdProfile(motion::sysmon::Stat::NetTx).criticalDefault;
