...
public:
  ...
  [[nodiscard]] const motion::config::LegacyConfigIssues& legacyConfigIssues() const noexcept {
    return m_legacyConfigIssues;
  }

  // The optional label is used only for opt-in reload profiling (MOTION_PROFILE);
...
private:
  ...
  [[nodiscard]] motion::config::schema::Diagnostics diagnosticsForOverrides(const toml::table& overrides) const;
  [[nodiscard]] bool validateOverrideMutation(
      const toml::table& candidateOverrides, const toml::table* baselineOverrides = nullptr,
      const motion::config::schema::Diagnostics* candidateDiagnostics = nullptr
  );
  ...
  void updateLegacyConfigIssues(motion::config::LegacyConfigIssues issues);
  ...
  motion::config::LegacyConfigIssues m_legacyConfigIssues;
