...
namespace {
  ...
  motion::system::cpu_temp::ProbeResult readCpuTempSensor(const SystemConfig::MonitorConfig& config) {
    try {
      return motion::system::cpu_temp::read("/sys/class/hwmon", "/sys/class/thermal", config.cpuTempSensorPath);
    } catch (...) {
      return motion::system::cpu_temp::ProbeResult{
...
struct SystemMonitorService::IntelGpuReader {
  IntelGpuReader() {
    m_devices = motion::system::intel_gpu::findDevices();
    ...
    std::ranges::stable_partition(m_devices, [](const motion::system::intel_gpu::Device& device) {
      return motion::system::intel_gpu::readVram(device).has_value();
    });
  }
  ...
  [[nodiscard]] std::string usageSource() const {
    return m_devices.empty() ? std::string{} : motion::system::intel_gpu::usageSource(m_devices.front());
  }

  [[nodiscard]] std::optional<motion::system::intel_gpu::UsageReading> readUsage() {
    for (const auto& device : m_devices) {
  ...
  [[nodiscard]] std::optional<motion::system::intel_gpu::VramReading> readVram() const {
    for (const auto& device : m_devices) {
      if (const auto reading = motion::system::intel_gpu::readVram(device); reading.has_value()) {
        return reading;
      }
    }
    return std::nullopt;
...
private:
  std::vector<motion::system::intel_gpu::Device> m_devices;
  std::unordered_map<std::string, motion::system::intel_gpu::UsageSampler> m_samplers;
};
