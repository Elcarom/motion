...
// MOTION_PROFILE env var so normal runs stay silent; set it to any non-empty
// value to surface timing lines (e.g. `MOTION_PROFILE=1 motion`).
namespace motion::profiling {

  // Evaluated once on first use.
  inline bool enabled() {
    static const bool value = [] {
      const char* v = std::getenv("MOTION_PROFILE");
