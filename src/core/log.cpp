...
namespace {
  ...
  constexpr const char* kLogLevelEnv = "MOTION_LOG_LEVEL";
...
void initLogFile() {
  ...
  if (cacheHome != nullptr && cacheHome[0] != '\0') {
    dir = std::string(cacheHome) + "/motion";
  } else if (home != nullptr && home[0] != '\0') {
    dir = std::string(home) + "/.cache/motion";
  } else {
  ...
  const std::string logPath = dir + "/motion.log";
  const std::string backupPath = dir + "/motion.log.1";
