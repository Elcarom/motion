...
namespace scripting {
  ...
  // plugin's UI entries (widgets, panels) consume it via motion.state.
  ...
  public:
    ...
    // list is read via motion.outputs() inside the callback.
    void onOutputChange();

  private:
    // A service is reachable by IPC via the `all` target (it is a singleton with no
    // output): `motion msg plugin <author/plugin:entry> all <event> [payload]`.
