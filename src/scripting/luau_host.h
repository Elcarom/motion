...
public:
  ...
  // Registers a `motion.state.watch` callback with the shared store (the runtime
  ...
  // motion.state.* — host-mediated per-plugin shared data.
  ...
  // motion.runStream — run a long-lived process and deliver each stdout line to a
  ...
  // it lives on motion.* and works for every entry type, including headless services.
