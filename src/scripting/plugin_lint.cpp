    ...
    ScanResult scanGetConfigCalls(std::string_view src) {
      ...
      while (i < n) {
        ...
        if (isIdentChar(c) && (i == 0 || !isIdentChar(src[i - 1]))) {
          ...
          // getConfig is universal and lives only under motion.*. Record the old
...
namespace motion::plugins {
  namespace {
    ...
    constexpr const char* kHelpText =
        "Usage: motion plugins <command> [paths]\n"
        "\n"
        "Offline tools for plugin authors (no running shell required).\n"
        "To manage installed plugins on the running instance, use 'motion msg plugins'.\n"
        "\n"
        "Commands:\n"
        "  lint [path ...]\n"
    ...
    void printReport(const scripting::PluginLintReport& report, int& errors, int& warnings) {
      ...
      for (const auto& f : report.findings) {
        ...
        case Kind::ObsoleteConfigAccessor:
          std::println("  error  {}  '{}' was removed; use motion.getConfig", where, f.key);
          ++errors;
          break;
  ...
  int runCli(int argc, char* argv[]) {
    // argv[0] = "motion", argv[1] = "plugins"; commands start at argv[2].
    if (argc < 3) {
