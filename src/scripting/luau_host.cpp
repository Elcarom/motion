...
namespace {
  Logger kLog{"luau"};
  constexpr const char* kHostKey = "__motion_host";
  ...
  int luau_notifyError(lua_State* L) {
    ...
    if (auto* host = hostForState(L)) {
      ...
      notify::error("Motion", title, body);
    }
    return 0;
  }
  ...
  const luaL_Reg kMotionStateLib[] = {
      {"set", luau_state_set},
      {"get", luau_state_get},
  ...
  const luaL_Reg kMotionStringLib[] = {
  ...
  void registerMotionLib(lua_State* L) {
    luaL_register(L, "motion", kMotionBaseLib);
    // motion.state = { set, get, watch }
    lua_createtable(L, 0, 0);
    luaL_register(L, nullptr, kMotionStateLib);
    lua_setfield(L, -2, "state");
    // motion.json = { decode, encode }
    lua_createtable(L, 0, 0);
    luaL_register(L, nullptr, kMotionJsonLib);
    lua_setfield(L, -2, "json");
    // motion.string = { trim, urlEncode, urlDecode }
    lua_createtable(L, 0, 0);
    luaL_register(L, nullptr, kMotionStringLib);
