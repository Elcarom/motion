...
namespace {
  ...
  constexpr const char* kDaemonPipeEnv = "MOTION_DAEMON_PIPE_FD";
  int g_daemonPipe = -1;
  ...
  int runTopLevelFlag(const char* flag) {
    if (std::strcmp(flag, "--version") == 0 || std::strcmp(flag, "-v") == 0) {
      const std::string version = motion::build_info::displayVersion();
      std::println("motion {}", version);
      return 0;
    }
    if (std::strcmp(flag, "--help") == 0 || std::strcmp(flag, "-h") == 0) {
      std::println(
          ...
          "                   Run 'motion msg --help' for available commands\n"
          "  theme <image>    Generate a color palette from an image\n"
          "                   Run 'motion theme --help' for options\n"
          "  config <command> Validate config and support/replay helpers\n"
          "                   Run 'motion config --help' for options\n"
          "  plugins <cmd>    Offline plugin author tools (lint)\n"
          "                   Run 'motion plugins --help' for options\n"
          "\n"
          "For more information, see README.md and the docs/ directory."
      );
      return 0;
    }
    return -1;
  ...
  int runShell() {
    ...
    if (!instanceLock.tryAcquire()) {
      std::println(stderr, "error: motion is already running");
...
int main(int argc, char* argv[]) {
...
#if defined(__GLIBC__) && !defined(MOTION_USE_JEMALLOC)
  ...
  const auto migration = motion::legacy::migrateNoctaliaProfile();
  for (const auto& item : migration.migrated) {
    std::println(stderr, "migration: {}", item);
  }
  for (const auto& warning : migration.warnings) {
    std::println(stderr, "migration warning: {}", warning);
  }
  ...
  if (argc >= 2) {
    if (std::strcmp(argv[1], "theme") == 0)
      return motion::theme::runCli(argc, argv);
    if (std::strcmp(argv[1], "msg") == 0)
      return motion::ipc::runCli(argc, argv);
    if (std::strcmp(argv[1], "config") == 0)
      return motion::config::runCli(argc, argv);
    if (std::strcmp(argv[1], "dmenu") == 0)
      return motion::launcher::runDmenuCli(argc, argv);
    if (std::strcmp(argv[1], "plugins") == 0)
      return motion::plugins::runCli(argc, argv);
  }
  ...
  if (shouldDaemonize && !isDaemonChild) {
    ...
    if (result == SpawnResult::Parent) {
      ...
      if (daemonResult == 0) {
        std::println("motion started [pid: {}]", pid);
      }
      return daemonResult;
    }
  }
