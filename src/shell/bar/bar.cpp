...
void Bar::reload() {
  motion::profiling::ScopedTimer t(kLog, "bar: reload (all instances)");
...
void Bar::createInstance(const WaylandOutput& output, std::size_t barIndex, const BarConfig& barConfig) {
  ...
  auto surfaceConfig = LayerSurfaceConfig{
      .nameSpace = "motion-bar-" + barConfig.name,
...
void Bar::rebuildInstanceContents(BarInstance& instance, const BarConfig& newConfig) {
  motion::profiling::ScopedTimer t(kLog, std::format("bar: rebuild contents [{}]", newConfig.name));
