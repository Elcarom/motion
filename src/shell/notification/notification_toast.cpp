...
namespace {
  ...
  constexpr std::string_view kMotionGlyphIconPrefix = "motion-glyph:";
  constexpr std::string_view kLegacyGlyphIconPrefix = "noctalia-glyph:";
  ...
  std::filesystem::path remoteIconCachePath(std::string_view url) {
    const std::filesystem::path cacheDir = std::filesystem::path(FileUtils::cacheDir()) / "notification-icons";
...
InputArea* NotificationToast::buildCard(
  ...
  *outCardForeground = foreground.get();
  ...
  if (entry.icon.has_value()) {
    ...
    if (rawIcon.size() > kMotionGlyphIconPrefix.size()
        && std::string_view(rawIcon.data(), kMotionGlyphIconPrefix.size()) == kMotionGlyphIconPrefix) {
      const std::string_view glyphName(
          rawIcon.data() + kMotionGlyphIconPrefix.size(), rawIcon.size() - kMotionGlyphIconPrefix.size()
      );
      if (!glyphName.empty()) {
        iconSlot->addChild(
            ui::glyph({
...
std::string NotificationToast::resolveNotificationIconPath(const PopupEntry& entry) {
  ...
  if (entry.icon->size() > kMotionGlyphIconPrefix.size()
      && std::string_view(entry.icon->data(), kMotionGlyphIconPrefix.size()) == kMotionGlyphIconPrefix) {
    return {};
  }
