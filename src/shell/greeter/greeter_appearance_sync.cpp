...
namespace {
  ...
  [[nodiscard]] std::filesystem::path makeStagingDirectory() {
    ...
    const auto staging = base / "motion-greeter-sync";
    std::error_code ec;
  ...
  [[nodiscard]] bool writeManifest(
    ...
    putPaletteColor(palette, "primary_container", ::palette.primaryContainer);
    putPaletteColor(palette, "on_primary_container", ::palette.onPrimaryContainer);
    putPaletteColor(palette, "secondary", ::palette.secondary);
    putPaletteColor(palette, "on_secondary", ::palette.onSecondary);
    putPaletteColor(palette, "secondary_container", ::palette.secondaryContainer);
    putPaletteColor(palette, "on_secondary_container", ::palette.onSecondaryContainer);
    putPaletteColor(palette, "tertiary", ::palette.tertiary);
    putPaletteColor(palette, "on_tertiary", ::palette.onTertiary);
    putPaletteColor(palette, "tertiary_container", ::palette.tertiaryContainer);
    putPaletteColor(palette, "on_tertiary_container", ::palette.onTertiaryContainer);
    putPaletteColor(palette, "error", ::palette.error);
    putPaletteColor(palette, "on_error", ::palette.onError);
    putPaletteColor(palette, "error_container", ::palette.errorContainer);
    putPaletteColor(palette, "on_error_container", ::palette.onErrorContainer);
    putPaletteColor(palette, "background", ::palette.background);
    putPaletteColor(palette, "on_background", ::palette.onBackground);
    ...
    putPaletteColor(palette, "surface_dim", ::palette.surfaceDim);
    putPaletteColor(palette, "surface_bright", ::palette.surfaceBright);
    putPaletteColor(palette, "surface_container_lowest", ::palette.surfaceContainerLowest);
    putPaletteColor(palette, "surface_container_low", ::palette.surfaceContainerLow);
    putPaletteColor(palette, "surface_container", ::palette.surfaceContainer);
    putPaletteColor(palette, "surface_container_high", ::palette.surfaceContainerHigh);
    putPaletteColor(palette, "surface_container_highest", ::palette.surfaceContainerHighest);
    putPaletteColor(palette, "surface_tint", ::palette.surfaceTint);
    putPaletteColor(palette, "outline", ::palette.outline);
    putPaletteColor(palette, "outline_variant", ::palette.outlineVariant);
    putPaletteColor(palette, "inverse_surface", ::palette.inverseSurface);
    putPaletteColor(palette, "inverse_on_surface", ::palette.inverseOnSurface);
    putPaletteColor(palette, "inverse_primary", ::palette.inversePrimary);
    putPaletteColor(palette, "scrim", ::palette.scrim);
    ...
    if (!installedWallpaperName.empty()) {
      wallpaper["path"] = (std::filesystem::path("/var/lib/motion-greeter") / installedWallpaperName).string();
...
namespace greeter {
  ...
  bool appearanceSyncAvailable(const ShellGreeterSyncConfig& greeterSync) noexcept {
    return programExists(kGreeterName, {"/usr/bin/motion-greeter", "/usr/local/bin/motion-greeter"})
  ...
  void registerIpc(
    ...
    ipc.registerHandler(
        ...
        [&config, resolvedThemeMode = std::move(resolvedThemeMode), platform,
         logindOnSystemBus = std::move(logindOnSystemBus)](const std::string& args) -> std::string {
          ...
          if (!appearanceSyncAvailable(config.config().shell.greeterSync)) {
            return "error: motion greeter is not installed\n";
          }
        ...
        "greeter-sync", "Sync wallpaper, colors, and monitor layout to Motion Greeter"
    );
  }
