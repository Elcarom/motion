...
namespace {
  ...
  constexpr auto kAgentObjectPath = "/org/motion/PolkitAuthenticationAgent";
...
using MotionPolkitListener = struct _MotionPolkitListener {
...
using MotionPolkitListenerClass = struct _MotionPolkitListenerClass {
  PolkitAgentListenerClass parent_class;
};

static void motion_polkit_listener_initiate_authentication(
...
static gboolean motion_polkit_listener_initiate_authentication_finish(
    PolkitAgentListener* listener, GAsyncResult* result, GError** error
);
static void motion_polkit_request_cancelled(GCancellable* cancellable, gpointer user_data) noexcept;

G_DEFINE_TYPE(MotionPolkitListener, motion_polkit_listener, POLKIT_AGENT_TYPE_LISTENER)

static void motion_polkit_listener_init(MotionPolkitListener* self) {
  self->owner = nullptr;
...
static void motion_polkit_listener_class_init(MotionPolkitListenerClass* klass) {
  auto* listenerClass = POLKIT_AGENT_LISTENER_CLASS(klass);
  listenerClass->initiate_authentication = motion_polkit_listener_initiate_authentication;
  listenerClass->initiate_authentication_finish = motion_polkit_listener_initiate_authentication_finish;
}
...
static void motion_polkit_listener_initiate_authentication(
  ...
  guardPolkitCallback("initiate_authentication", [&]() {
    auto* self = reinterpret_cast<MotionPolkitListener*>(listener);
    ...
    if (cancellable != nullptr) {
      request->cancelHandlerId =
          g_cancellable_connect(cancellable, G_CALLBACK(motion_polkit_request_cancelled), request.get(), nullptr);
    }
...
static void motion_polkit_request_cancelled(GCancellable* /*cancellable*/, gpointer user_data) noexcept {
  guardPolkitCallback("request_cancelled", [&]() {
    ...
    auto* listener = source != nullptr ? reinterpret_cast<MotionPolkitListener*>(source) : nullptr;
...
struct PolkitAgent::Impl {
  ...
  MotionPolkitListener* listener = nullptr;
  ...
  Impl() : context(g_main_context_default()) {
    listener = static_cast<MotionPolkitListener*>(g_object_new(motion_polkit_listener_get_type(), nullptr));
    listener->owner = this;
