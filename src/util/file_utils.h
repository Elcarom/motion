...
namespace FileUtils {
  ...
  [[nodiscard]] inline const char* preferredEnv(const char* current, const char* legacy) noexcept {
    if (const char* value = std::getenv(current); value != nullptr && value[0] != '\0') {
      return value;
    }
  ...
  [[nodiscard]] inline std::string configDir() {
    if (const char* base = preferredEnv("MOTION_CONFIG_HOME", "NOCTALIA_CONFIG_HOME"); base != nullptr) {
      return std::string(base) + "/motion";
    }
    if (const char* xdg = std::getenv("XDG_CONFIG_HOME"); xdg != nullptr && xdg[0] != '\0') {
      return std::string(xdg) + "/motion";
    }
    if (const char* home = std::getenv("HOME"); home != nullptr && home[0] != '\0') {
      return std::string(home) + "/.config/motion";
    }
    return {};
  }

  [[nodiscard]] inline std::string stateDir() {
    if (const char* base = preferredEnv("MOTION_STATE_HOME", "NOCTALIA_STATE_HOME"); base != nullptr) {
      return std::string(base) + "/motion";
    }
    ...
    if (const char* home = std::getenv("HOME"); home != nullptr && home[0] != '\0') {
      return std::string(home) + "/.local/state/motion";
    }
    return {};
  }

  [[nodiscard]] inline std::string dataDir() {
    if (const char* base = preferredEnv("MOTION_DATA_HOME", "NOCTALIA_DATA_HOME"); base != nullptr) {
      return std::string(base) + "/motion";
    }
    ...
    if (const char* home = std::getenv("HOME"); home != nullptr && home[0] != '\0') {
      return std::string(home) + "/.local/share/motion";
    }
    return {};
  }

  [[nodiscard]] inline std::string privateCacheFallback() {
    static const std::string fallback = [] {
      ...
      if (::mkdir(root.c_str(), 0700) == 0
        ...
        if (::lstat(root.c_str(), &st) == 0 && S_ISDIR(st.st_mode) && st.st_uid == uid && (st.st_mode & 0077) == 0) {
          return (root / "cache").string();
        }
      }
      ...
      return std::string("/dev/null/motion-cache-unavailable");
    }();
    return fallback;
  }

  // Re-fetchable and generated files. A legacy override is accepted only as a
  // compatibility bridge; all new paths remain under a Motion directory. When
  // no home can be resolved, keep the fallback private to the current uid.
  [[nodiscard]] inline std::string cacheDir() {
    if (const char* base = preferredEnv("MOTION_CACHE_HOME", "NOCTALIA_CACHE_HOME"); base != nullptr) {
      return std::string(base) + "/motion";
    }
    ...
    if (const char* home = std::getenv("HOME"); home != nullptr && home[0] != '\0') {
      return std::string(home) + "/.cache/motion";
    }
    return privateCacheFallback();
  }
  ...
  // materialized runtime dir) and honors the MOTION_STATE_HOME override.
