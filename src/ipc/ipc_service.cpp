...
IpcService::~IpcService() {
  const bool ownedSocket = m_listenFd >= 0;
  if (ownedSocket) {
    ::close(m_listenFd);
    m_listenFd = -1;
  }
  if (ownedSocket && !m_socketPath.empty()) {
...
bool IpcService::start() {
  ...
  if (::lstat(m_socketPath.c_str(), &existing) == 0) {
    ...
    if (::unlink(m_socketPath.c_str()) != 0) {
      kLog.warn("IPC disabled: failed to remove stale socket: {}", std::strerror(errno));
      ::close(fd);
      return false;
    }
  } else if (errno != ENOENT) {
    kLog.warn("IPC disabled: failed to inspect socket path: {}", std::strerror(errno));
    ::close(fd);
    return false;
  }
  ...
  const int bindResult = ::bind(fd, reinterpret_cast<const sockaddr*>(&addr), sizeof(addr));
  ...
  if (::chmod(m_socketPath.c_str(), 0600) != 0) {
    kLog.warn("IPC disabled: chmod() failed: {}", std::strerror(errno));
    ::close(fd);
    ::unlink(m_socketPath.c_str());
    return false;
  }
...
std::string IpcService::buildHelp() const {
  ...
  std::string out = "Usage: motion msg <command> [args]\n\nCommands:\n";
...
std::string IpcService::executeParsed(const std::string& command, const std::string& args) const {
  ...
  if (it == m_handlers.end()) {
    return "error: unknown command (try: motion msg --help)\n";
  }
  return it->second.fn(args);
}

std::string IpcService::resolveSocketPath() { return motion::runtime_path::path("sock"); }
