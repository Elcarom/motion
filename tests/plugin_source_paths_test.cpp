...
int main() {
  ::setenv("MOTION_STATE_HOME", "/tmp/motion-path-test-state", 1);
  ::setenv("MOTION_DATA_HOME", "/tmp/motion-path-test-data", 1);
  ...
  ok = expect(
           scripting::plugin_paths::registryRoot(pathSource).string().ends_with("/dev/motion-plugins"),
  ...
  ok = expect(
      ...
      && ok;
  ok = expect(
           !scripting::plugin_paths::pathIsInside("/tmp/motion", "/tmp/motion"),
           "parent path must not count as inside itself"
       )
      && ok;
  ok = expect(
           !scripting::plugin_paths::pathIsInside("/tmp/motion-other/a", "/tmp/motion"),
