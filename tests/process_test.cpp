...
namespace {
  ...
  bool syncAppliesEnvOverrides() {
    ::setenv("MOTION_PROCESS_UNSET_TEST", "parent", 1);

    process::RunOptions options;
    options.env.push_back({"MOTION_PROCESS_SET_TEST", "child"});
  ...
  bool detachedAsyncInheritsLaunchEnvironment() {
    const std::filesystem::path outPath =
        std::filesystem::temp_directory_path() / ("motion_process_env_test_" + std::to_string(::getpid()));
    std::error_code ec;
    ...
    for (int i = 0; i < 50; ++i) {
      ...
      if (contents == "/tmp/motion test/wallpaper.png\nDP-1") {
        break;
      }
    ...
    std::filesystem::remove(outPath, ec);
    return expect(contents == "/tmp/motion test/wallpaper.png\nDP-1", "detached async env was not visible in child");
  }
