...
namespace {
  ...
  std::filesystem::path makeTempDir() {
    std::string pattern = (std::filesystem::temp_directory_path() / "motion-ipc-service-XXXXXX").string();
...
int main() {
  ...
  constexpr const char* kWaylandDisplay = "motion-ipc-service-test";
  ...
  // A failed start must never remove an unrelated path during destruction.
  constexpr const char* kUnsafeDisplay = "motion-ipc-unsafe-test";
  assert(::setenv("WAYLAND_DISPLAY", kUnsafeDisplay, 1) == 0);
  const auto unsafePath = runtimeDir / ("motion-" + std::string(kUnsafeDisplay) + ".sock");
  {
    std::ofstream unsafeFile(unsafePath);
    unsafeFile << "do-not-delete";
  }
  {
    IpcService unsafeIpc;
    assert(!unsafeIpc.start());
  }
  assert(std::filesystem::is_regular_file(unsafePath));
