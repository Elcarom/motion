...
namespace {
  ...
  uint32_t token(const motion::theme::GeneratedPalette& palette, const std::string& name) {
    const auto it = palette.dark.find(name);
    return it == palette.dark.end() ? 0u : it->second;
  }

  double saturation(uint32_t argb) {
    const auto color = motion::theme::Color::fromArgb(argb);
  ...
  bool checkSchemeStrings() {
    ...
    ok = expect(!motion::theme::schemeFromString("softened").has_value(), "rejects non-canonical soft alias") && ok;
    return ok;
  }
  ...
  bool checkSoftGeneration() {
    ...
    const auto faithful = motion::theme::generateCustom(rgb, Scheme::Faithful);
    const auto soft = motion::theme::generateCustom(rgb, Scheme::Soft);
    const auto muted = motion::theme::generateCustom(rgb, Scheme::Muted);
