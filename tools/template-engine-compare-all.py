...
PYTHON_PROCESSOR = (
    Path.home() / "Development/misc/motion/motion-shell/Scripts/python/src/theming/template-processor.py"
)
PYTHON_THEMING_DIR = Path.home() / "Development/misc/motion/motion-shell/Scripts/python/src/theming"
TEMPLATE_ROOT = Path.home() / "Development/misc/motion/motion-shell/Assets/Templates"
PREDEFINED_SCHEME_JSON = (
    Path.home() / "Development/misc/motion/motion-shell/Assets/ColorScheme/Motion-default/Motion-default.json"
)
...
def main() -> int:
    ...
    if not MOTION_BIN.exists():
        print(f"error: motion binary not found: {MOTION_BIN}", file=sys.stderr)
        return 1
