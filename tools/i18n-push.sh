...
# Push translations to an explicitly configured translation API
# Usage: MOTION_TRANSLATION_PUSH_SECRET=your_secret ./i18n-push.sh [--overwrite] [--lang <code>] [/path/to/assets/translations]
...
# Configuration
API_URL="${TRANSLATION_API_URL:?Error: TRANSLATION_API_URL must name the translation service}"
PROJECT_SLUG="${TRANSLATION_PROJECT:-motion}"

# Check for secret
if [ -z "$MOTION_TRANSLATION_PUSH_SECRET" ]; then
    echo "Error: MOTION_TRANSLATION_PUSH_SECRET environment variable is required"
    exit 1
fi
...
RESPONSE=$(echo "$COMBINED_JSON" | curl -s -w "\n%{http_code}" -X POST \
    "$PUSH_URL" \
    -H "Authorization: Bearer $MOTION_TRANSLATION_PUSH_SECRET" \
