#!/usr/bin/env python3
"""
Analyze Motion's C++ palette generator against Python + matugen references.
...
Three backends:
  - Motion  : ../build-debug/motion theme <img> --scheme <s> --dark
  - Python    : the upstream reference in motion-shell/Scripts/python
...
MOTION_BIN = REPO_DIR / "build-debug" / "motion"

# Absolute path into the upstream Python reference.
PYTHON_THEMING_DIR = Path.home() / "Development/misc/motion/motion-shell/Scripts/python/src/theming"
...
def run_motion(image: Path, scheme: str) -> dict | None:
    try:
        out = subprocess.run(
            [str(MOTION_BIN), "theme", str(image),
             "--scheme", scheme, "--dark"],
            capture_output=True, text=True, check=True,
        )
        return json.loads(out.stdout)
    except Exception as e:
        print(f"  motion {scheme}: {e}", file=sys.stderr)
        return None
...
def compare_m3(image: Path, scheme: str, has_matugen: bool) -> list[int]:
    ...
    hdr = f"  {'Token':<26} {'Python':<10} {'Matugen':<10} {'Motion':<10} {'Δ Py↔Noct':<14} {'Δ Mat↔Noct':<14}"
    print(hdr)
    print("  " + "─" * (len(hdr) - 2))
...
def compare_custom(image: Path, scheme: str) -> list[int]:
    ...
    hdr = f"  {'Token':<26} {'Python':<10} {'Motion':<10} {'Δ Py↔Noct':<14}"
    print(hdr)
    print("  " + "─" * (len(hdr) - 2))
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compare Motion C++ palette generator vs Python/matugen"
    )
    ...
    if not MOTION_BIN.exists():
        print(f"error: motion not built: {MOTION_BIN}", file=sys.stderr)
        return 1
